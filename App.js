...
const Btn=({title,type})=>(
  <TouchableOpacity onPress={()=>calculate(title)} style={{padding:10,borderRadius:10,elevation:2,
    ...
    height:70,width:70,margin:16
  }}>
  <Text style={{fontSize:37,color:"black",textAlign:"center",textAlignVertical:"center",
...
return getColor(colors.light,colors.dark)
  ...
  return (
    <View style={{
     ...
     <Switch style={{marginRight:20}} value={darkTheme} onValueChange={()=>setDarkTheme(!darkTheme)} 
     thumbColor={getColor(colors.dark,colors.light)}
    //  trackColor={{true: colors.light2 , false: colors.dark2}}
     />
     <Text style={{fontSize:40, color:getColor(colors.light,colors.dark),width:"100%",textAlign:"right",paddingRight:20,paddingBottom:20}} >{result}</Text> 


<View style={{flexDirection: "row", flexWrap: "wrap",justifyContent:"center", backgroundColor:getColor(colors.dark1,colors.light1),borderTopRightRadius:20,borderTopLeftRadius:20,position:'absolute',bottom:0}}>
